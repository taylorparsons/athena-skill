# Customer Requests (append-only)

## CR-20260415-0900

### Metadata
- **Date:** 2026-04-15 09:00
- **Source:** chat

### Request (verbatim):
update the version to 2.0.0

## CR-20260414-1945

### Metadata
- **Date:** 2026-04-14 19:45
- **Source:** chat

### Request (verbatim):
curl -fsSL https://claude.ai/install.sh | bash

Make sure this work in tracked by Athena

## CR-20260414-1700

### Metadata
- **Date:** 2026-04-14 17:00
- **Source:** chat

### Request (verbatim):
create a plan for a sweeper agent to make or another functionality that looks for claude projects that are using too many tokens due to large static files in the docs/ folder or other cases that could be optimzed. Have this run once a week before 4 AM Pacific time

## CR-20260414-1600

### Metadata
- **Date:** 2026-04-14 16:00
- **Source:** chat

### Request (verbatim):
can you create a script to find all claude project and migrate them to the latest version of athena with owl agent ?

## CR-20260413-1200

### Metadata
- **Date:** 2026-04-13 12:00
- **Source:** chat

### Request (verbatim):
Create INDEX.md for token-optimized feature loading and implement Owl of Athena agent to manage archived features. Reduce token overhead by 80-90% by only loading active features. Add Git hooks for automatic INDEX.md maintenance. Include progress.txt archival to reduce session startup cost.

## CR-20260211-0908

### Metadata
- **Date:** 2026-02-11 09:08
- **Source:** chat

### Request (sanitized for public repo):
Create this repository under a local filesystem path.

### Notes
- Local absolute path removed for public-share hygiene.

## CR-20260211-0939

### Metadata
- **Date:** 2026-02-11 09:39
- **Source:** chat

### Request (verbatim):
I need a clean repo for this share there are artifacts I copied from another repo into here as an example of useing Ralph but I want to share only for the creation of this repo.  I do want it to include the ralph artifacts in the docs/folder but only scoped to this public share

### Notes
- Remove imported artifacts from other repos.
- Keep RALPH docs artifacts but scope them to this repository only.

## CR-20260211-1004

### Metadata
- **Date:** 2026-02-11 10:04
- **Source:** chat

### Request (verbatim):
remove the Daisy skill from this version of $ralph

### Notes
- Remove Daisy mentions from the public-share RALPH repository content.

## CR-20260211-1010

### Metadata
- **Date:** 2026-02-11 10:10
- **Source:** chat

### Request (verbatim):
questions
1. truely clean 
2. remove local strings

also hardening pass
1. yes
2. yes 
3. SECURITY.md 
4, Add a MIT lisense open source

### Notes
- Rewrite repository history for a truly clean public baseline.
- Remove local absolute path strings from tracked docs.
- Add basic security/public hardening artifacts.

## CR-20260211-1019

### Metadata
- **Date:** 2026-02-11 10:19
- **Source:** chat

### Request (verbatim):
yes do the low priority items, there is one code owner

### Notes
- Complete remaining low-priority hardening findings.
- Configure a single repository code owner.

## CR-20260211-1325

### Metadata
- **Date:** 2026-02-11 13:25
- **Source:** chat

### Request (verbatim):
codex resume 019c48d0-95a1-72f0-a8cb-6243bd8b3c2

### Notes
- Resume RALPH execution from current repository state.

## CR-20260211-1358

### Metadata
- **Date:** 2026-02-11 13:58
- **Source:** chat

### Request (verbatim):
did you update the repos $ralph ? if not do so now, if true check-in with comments

yes remove it

### Notes
- Complete repo updates and create a local check-in with traceability comments.
- Keep public install guidance focused on `ralph` only.

## CR-20260212-1646

### Metadata
- **Date:** 2026-02-12 16:46
- **Source:** chat

### Request (verbatim):
I have a multi ask and use $ralph to help process
1. I want to rename the skill from ralph to athen but only on the git repo for now 
2. I want to rename git repro project from ralph-traceability-loop to athena-skill 
3.I want all code and documentation in the current git to be updated to reference athena in place of ralph.

### Notes
- Request includes both `athen` and `athena`; interpretation recorded in decisions log.

## CR-20260213-0912

### Metadata
- **Date:** 2026-02-13 09:12
- **Source:** chat

### Request (verbatim):
create two more versions, I would love to see an info logical flow from the calling of athena and a request with the loops to the completion of a checkin 
2. clear instructions how to install it for both claude and codex embedded in the text a copy and paste code blocks is linkedIn supports that in a Post

### Notes
- User asked for two additional versions suitable for LinkedIn posting.

## CR-20260213-1958

### Metadata
- **Date:** 2026-02-13 19:58
- **Source:** chat

### Request (verbatim):
create one more using my personal writing style

You want the honest version, not the LinkedIn bio version. Good.

Here is your writing style, based on how you actually write when you are not performing for a boardroom.

## Core Traits

### 1. Direct and concrete

You do not tolerate fluff.
You prefer statements like:

* “Here is the problem.”
* “This is the constraint.”
* “This is what we learned.”

You strip out filler words. You dislike vague boosters. You want specifics, numbers, timelines, tradeoffs.

### 2. Structured thinking

You think in frameworks.

* Hypothesis
* Assumptions
* Risk
* Evidence
* Decision
* Confidence

Even when you are emotional, you organize it. Your writing reflects system design thinking. You rarely ramble. You scaffold.

### 3. Reflective, but grounded

When you shift into personal mode, you write with:

* Warmth
* Pride
* Quiet honesty
* Story over abstraction

You do not overshare for drama. You anchor reflection in action. “This is what happened. This is what I learned.”

### 4. Operator mindset

You write like someone who has shipped things.

You care about:

* Execution paths
* Traceability
* Failure modes
* Measurable outcomes

Even your creative projects read like a product spec disguised as a story.

### 5. Low tolerance for marketing tone

You avoid:

* Inflated adjectives
* Buzzwords
* Grand declarations without proof

If a sentence cannot survive scrutiny, you rewrite it.

### 6. Systems + humans

This is the interesting part.

You combine:

* Process rigor
* AI architecture thinking
* Enterprise realism

With:

* Care for people
* Loyalty
* Emotional continuity

You are analytical, but not sterile. You care who gets hurt, who benefits, and whether something is honest.

## How You Sound in Different Contexts

### Professional

Clear. Structured. Evidence-based. Calm authority. No hype.

### Personal

Warm. Narrative. Grounded in memory. Proud of people. Optimistic without pretending the world is simple.

### Technical

Precise. Traceable. Step-based. Concerned with edge cases.

## What Makes It Distinct

You do not write to impress.
You write to reduce ambiguity.

You want:

* Durable thinking
* Reusable frameworks
* Decisions that can be audited later

Most people write to sound smart.
You write to make something usable.

That difference matters.

### Notes
- User asked for one additional version using provided personal writing style traits.

## CR-20260213-1434

### Metadata
- **Date:** 2026-02-13 14:34
- **Source:** chat

### Request (verbatim):
update the readme.md this using this image [Image #1] at the top for a fast visual

### Notes
- Add a top-of-README visual using the user-provided diagram.

## CR-20260213-1435

### Metadata
- **Date:** 2026-02-13 14:34
- **Source:** chat

### Request (verbatim):
dont use this  untracked docs/
  athena-napkin-loop.svg.

### Notes
- Do not reference or depend on `docs/athena-napkin-loop.svg`.

## CR-20260213-1436

### Metadata
- **Date:** 2026-02-13 14:35
- **Source:** chat

### Request (verbatim):
yes my request and also while you are in here make sure the text in the readme.md is using my $taylor-style-voice

### Notes
- Rewrite README text in Taylor style voice while preserving technical correctness.

## CR-20260213-1448

### Metadata
- **Date:** 2026-02-13 14:48
- **Source:** chat

### Request (verbatim):
just add the image to the readme.md this is not correct it things like conext: decision: execution.
that is too literal just leave the new image

### Notes
- Scope correction: keep only the new top image and remove literal style labels from README text.

## CR-20260213-1452

### Metadata
- **Date:** 2026-02-13 14:52
- **Source:** chat

### Request (verbatim):
checkin a marge to remote

### Notes
- Interpret as: create a local check-in commit and merge/push to remote.

## CR-20260216-1317

### Metadata
- **Date:** 2026-02-16 13:17
- **Source:** chat

### Request (verbatim):
on on 1-5

### Notes
- Interpreted as instruction to implement findings 1-5 from the immediately preceding project review.

## CR-20260217-0846

### Metadata
- **Date:** 2026-02-17 08:46
- **Source:** chat

### Request (verbatim):
to the athena skill create an svg asset for skill icon that invokes the warrier

### Notes
- Interpreted `warrier` as `warrior` for the icon concept.

## CR-20260217-1417

### Metadata
- **Date:** 2026-02-17 14:17
- **Source:** chat

### Request (verbatim):
$skill-creator update athena install in .codex to inlude the /assets/athena-warrior-icon.svg

### Notes
- Update ATHENA install metadata/packaging so the warrior icon asset is included at install time.

## CR-20260217-1451

### Metadata
- **Date:** 2026-02-17 14:51
- **Source:** chat

### Request (verbatim):
address absolute paths

### Notes
- Remediate repository absolute-path audit findings in tracked docs.

## CR-20260217-1535

### Metadata
- **Date:** 2026-02-17 15:35
- **Source:** chat

### Request (verbatim):
$aipm-agentic-workflow create the documents for this Add a traceability linter that validates Sources, Verifies, and Implements references.
Consolidate overlapping merge-sync sections into one canonical checklist.
Make path-scoped staging the default in traceable commit helper usage.
Add schema validation for docs/progress.txt to reduce restore failures. process through $athena

this /Volumes/T9/code/pm_reflection/docs/specs/20260217-athena-hardening should be in the repo for Athena

here is the repo /Volumes/T9/code/SKILLS/athena the docs should be in

### Notes
- Scope is documentation artifacts in the ATHENA repo for the four hardening items.

## CR-20260217-1550

### Metadata
- **Date:** 2026-02-17 15:50
- **Source:** chat

### Request (verbatim):
compelte the work for T-001
continue
complete all of the work

### Notes
- Interpreted as implementation request to finish all remaining tasks for feature `20260217-athena-hardening-plan`.

## CR-20260218-0902

### Metadata
- **Date:** 2026-02-18 09:02
- **Source:** chat

### Request (verbatim):
update the version to 1.1.0

### Notes
- Apply version bump in the ATHENA repo.

## CR-20260218-1059

### Metadata
- **Date:** 2026-02-18 10:59
- **Source:** chat

### Request (verbatim):
what if we added a security check before git add that makes sure that this is not checked in: DS_Store, secrets, temp files, large
    artifacts
add that
use $athena to help

### Notes
- Add pre-stage guardrails so broad staging remains usable while blocking risky files.

## CR-20260219-1638

### Metadata
- **Date:** 2026-02-19 16:38
- **Source:** chat

### Request (verbatim):
apply in /Volumes/T9/code/SKILLS/athena/docs

### Notes
- Apply documentation readability updates in the source installation repository.

## CR-20260219-1639

### Metadata
- **Date:** 2026-02-19 16:39
- **Source:** chat

### Request (verbatim):
this is the source that I install from

### Notes
- Confirms `/Volumes/T9/code/SKILLS/athena` is the authoritative source repo.

## CR-20260219-1640

### Metadata
- **Date:** 2026-02-19 16:40
- **Source:** chat

### Request (verbatim):
you can leave the wrong path as is for now

### Notes
- Do not rollback prior `.codex`-path edits in this session.

## CR-20260219-1641

### Metadata
- **Date:** 2026-02-19 16:41
- **Source:** chat

### Request (verbatim):
I only want to apply this update moving forward

### Notes
- Scope moving forward changes to `/Volumes/T9/code/SKILLS/athena`.

## CR-20260219-1642

### Metadata
- **Date:** 2026-02-19 16:42
- **Source:** chat

### Request (verbatim):
use $athena to manage this chang request

### Notes
- Execute and track this work through the ATHENA loop.

## CR-20260219-1658

### Metadata
- **Date:** 2026-02-19 16:58
- **Source:** chat

### Request (verbatim):
Update the PRD so each one of the numbered references is a link to the document and location in the document

### Notes
- Make every numbered reference in `docs/PRD.md` a clickable link to its target artifact and location.

## CR-20260219-1700

### Metadata
- **Date:** 2026-02-19 17:00
- **Source:** chat

### Request (verbatim):
the firt link I tried is a 404 http://localhost:8765/docs/requests.md#cr-20260211-0939

### Notes
- Fix relative links in `docs/PRD.md` so they resolve in local markdown viewers and do not duplicate the `docs/` path segment.

## CR-20260219-1702

### Metadata
- **Date:** 2026-02-19 17:02
- **Source:** chat

### Request (verbatim):
the firt link I tried is a 404 http://localhost:8765/docs/requests.md#cr-20260211-0939

### Notes
- Confirm local preview link format and keep the fix moving forward.

## CR-20260220-1707

### Metadata
- **Date:** 2026-02-20 17:07
- **Source:** chat

### Request (verbatim):
the firt link I tried is a 404 http://localhost:8765/docs/requests.md#cr-20260211-0939
this is not correct http://localhost:8765/api/file?path=/Volumes/T9/code/SKILLS/athena/docs/decisions.md#d-20260219-1702
when you open the link it is realative
but this opening a a raw file
I am viewing the markdown in a reader http://localhost:8765/

### Notes
- Change all PRD numbered references to a portable form, retest each link, and keep only the reader-compatible target shape.

## CR-20260219-1710

### Metadata
- **Date:** 2026-02-19 17:10
- **Source:** chat

### Request (verbatim):
I only want reader-compatible links in PRD: plain links should open documents with location and not 404 or raw JSON/file.

### Notes
- Update numbered references to avoid 404/relative/raw JSON behavior in the markdown reader at `http://localhost:8765/`.

## CR-20260221-1252

### Metadata
- **Date:** 2026-02-21 12:52
- **Source:** chat

### Request (verbatim):
document the changes in $athena and check in to prepare to push to remote

### Notes
- Track this work in ATHENA artifacts and prepare a local, traceable commit for `skills/athena` template updates.

## CR-20260414-1100

### Metadata
- **Date:** 2026-04-14 11:00
- **Source:** chat

### Request (verbatim):
interesting - /Volumes/T9/code/agentic-browser/docs/index.md already existed and it is just an index for the docs/ given this I think we need to make an update so that Athena is using athena_index.md and owl will use athena_index.md in place index.md as this is likely to be common in more mature repos. Use Superpowers to plan and implement this change in this repo and on this computer when it is done

### Notes
- Filename chosen: `athena-index.md` (kebab-case)
- No migration needed — feature never released, only impacts athena repo and my_LLC (being reverted)
- Append-only logs untouched; progress-archive.txt left as historical record
- README existing-project bootstrap: single Claude prompt first, then 5 manual steps

## CR-20260414-1000

### Metadata
- **Date:** 2026-04-14 10:00
- **Source:** chat

### Request (verbatim):
now add this to this repo that will be used by other to install the update for claude

### Notes
- Context: we debugged and fixed a SessionStart hook error caused by `type: "agent"` being unsupported in Claude Code settings.json hooks.
- Fix: replace the agent hook with `type: "command"` mirroring the working Stop hook pattern.
- Scope: update README.md hook snippet + add a patch script for existing installs.

## CR-20260413-1400

### Metadata
- **Date:** 2026-04-13 14:00
- **Source:** chat

### Request (verbatim):
review the change to Athena on this branch and the addition of the new agent called Owl of Athena. Compare to main for full view

### Notes
- Code review of feature/index-based-archival branch against main.
- Review covers INDEX.md token optimization and Owl of Athena agent.

## CR-20260414-1327

### Metadata
- **Date:** 2026-04-14 13:27
- **Source:** chat

### Request (verbatim):
yes spec that out using athena in this athena project. Ideally Owl would read the docs and put in the memory before Athena got there and Athena just use the memory and keep the memory updated as she works.

### Notes
- Context: discussion of whether Athena or Owl writes to Claude Code auto-memory (~/.claude/projects/.../memory/).
- Neither currently does. Gap identified: Athena re-reads 6-7 docs/ files every session; Owl already has the data at SessionStart.
- Proposed: new `write-memory` command in owl.py, called at SessionStart after update-index, writes project_athena_active.md to Claude Code memory dir.
